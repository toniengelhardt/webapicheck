<template>
  <NuxtLayout>
    ...
    <ApiList v-else-if="displayMode === 'rows'" :apis="filteredAPIs" :source="source" />
    <ApiGrid v-else-if="displayMode === 'tiles'" :apis="filteredAPIs" :source="source" />
  </NuxtLayout>
</template>
...
const webApiStatuses = useState(props.source, (): { [key: keyof typeof webApiData]: boolean } => ({}))

if (props.source === 'sharedWebApiStatuses') {
  useSharedWebApis()
} else {
